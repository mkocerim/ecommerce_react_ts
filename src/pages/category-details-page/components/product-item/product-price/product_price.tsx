...
export default function ProductPrice(props: ProductPricePropsType) {
  if (props.variant.price === props.variant.originalPrice) {
    return <a className="product-price">${props.variant?.price}</a>;
  } else {
    let discountRate: number =
      (props.variant.originalPrice - props.variant.price) /
    ...
    return (
      <>
        <a className="product-price">${props.variant?.price}</a>
        <a className="discounted-price">${props.variant?.originalPrice}</a>
